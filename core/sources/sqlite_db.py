...
设计边界（重要，请务必读完）：
  ...
  它要求用户把本地微信库导出为「已解密的 .db 文件」，本工具再以只读方式读取这些文件。
  导出方式不限，用户可自行用自己信任的途径完成。

这样划分的原因：
  1) 内存扒密钥的手段会随微信版本更新而失效，绑进主流程会让工具随时变砖；
  2) 企业合规审查里，"读取已授权导出的数据文件" 比 "注入微信进程" 好解释得多；
  3) 责任边界清晰——取数授权由用户自行完成。
...
class DecryptedDbSource(ChatSource):
    ...
    hint = ("选择本机已解密导出的微信数据库压缩包（.zip）或解压后的文件夹"
            "（里面是 MicroMsg.db / MSG*.db 等文件）。本工具以只读方式读取，"
            "不解密、不读取微信进程内存。导出方式由用户自行掌握。")
    needs_path = True
    path_kind = "file"
