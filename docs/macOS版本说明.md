...
产物：`dist_mac/绿泡泡聊天记录总结.app`（可选 `.dmg`）。

### ⚠️ 未签名产物在别的 Mac 上打不开？先清隔离标记

我们的产物**没有 Apple 开发者签名**（也未公证）。从浏览器/网盘下载到别的 Mac 后，
文件会被打上 **quarantine（隔离）标记**，Gatekeeper 会拦下来：

| 现象 | 说明 |
|---|---|
| 双击 `.dmg` 弹「**已损坏，无法打开。你应该将它移到废纸篓**」 | **不是文件坏了** —— 是未签名 + 隔离标记 |
| 双击 `.dmg` 弹「无法打开，因为 Apple 无法检查其是否包含恶意软件」 | 同上 |
| `.dmg` 能挂载，但里面 `.app` 双击没反应 / 报"无法验证开发者" | 同样，`.app` 也带着隔离标记 |

**在目标 Mac 上执行这两条即可**（先清 dmg，再清装好的 app）：

```bash
xattr -cr ~/Downloads/绿泡泡聊天记录总结-arm64.dmg      # 文件名按实际改
# 把 .app 拖进「应用程序」之后：
xattr -cr "/Applications/绿泡泡聊天记录总结.app"
```

也可以用图形方式：系统设置 → 隐私与安全性 → 找到被拦的提示 → 点「**仍要打开**」。

> 想彻底免掉这一步，只有一条路：**申请 Apple Developer 账号做签名 + 公证**（走 Developer ID，
> 年费 99 美元）。我们这边无法替用户绕过 Gatekeeper —— 那正是它的设计目的。

**下载物选哪个（别选错架构）**：Apple 芯片（M 系列）用 `*-arm64.dmg`；
Intel Mac 用 `*-x86_64.dmg`。装错架构的表现是：`.app` 图标上出现**禁止符 ⃠**、双击打不开
（Rosetta 只能把 x86 翻译成 arm64，反不过来）。
